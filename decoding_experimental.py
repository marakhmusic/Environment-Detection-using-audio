...
def identify_v3_upload():
    SAMPLE_RATE = 44100
    print('inside identify_v3_upload')
    # Check if the requests has a file
    if 'file' in request.files:
        ...
        if file.filename == '':
            ...
            sample_bytes = utils.convert_to_PCM(file, out_Fs=SAMPLE_RATE)
            print('sample_bytes: ', len(sample_bytes))
            # convert the file into pcm samples
            utils.save_as_wav(sample_bytes, 'recorded_encoded_file.wav', out_Fs=SAMPLE_RATE)
            return sample_bytes
...
def find_bpm(audio_file):
    ...
    print("The tempo data type is", type(tempo))
    return tempo
...
def in_decoded_audio(audio):
    ...
    temp = max(audio)
    print("The position of the maximum value is at ", np.argmax(temp))
    print("The length of the audio file is", len(audio))
    threshold = temp - 0.01
    print(audio[257])
    for sample in range(len(audio)):
        if audio[sample]>=threshold:
            print("This is the sample", sample)
            new_audio = audio[sample+99:sample+99+44100]
            break
...
if __name__ == '__main__':
    ...
    tempo_array = in_decoded_audio('/Users/mansoorkhan/ToneTag_Experiments/Python Projects/Environment_detection/App_Development/Flask_App/static/music/1011011100.wav')
    print(tempo_array)
